...
import { constant, checkIndexBounds, computeIndex, getDisplaySameSlide } from "../core/index";
...
class SwipeableViews extends React.Component {
	...
	// 	return { indexFromNextProps: nextProps.index };
	// }

	static getDerivedStateFromProps(nextProps, prevState) {
		const { index } = nextProps;
		if (typeof index === "number" && index != prevState.index) {
			return { index: nextProps.index,np:nextProps }; //, displaySameSlide: getDisplaySameSlide(nextProps, index), indexLatest: index };
		} else {
			return null;
		}
	}
	...
	componentDidUpdate(prevProps, prevState,nextProps) {
		...
		if (typeof index === "number" && index !== prevState.index) {
			...
			this.setState({
				// If true, we are going to change the children. We shoudn't animate it.
				displaySameSlide: getDisplaySameSlide(this.props,this.state.np),
				indexLatest: index
			});
		}
	}
			...
			>
				...
				>
					{React.Children.map(children, (child, indexChild) => {
									...
									//console.log("this is indexChild ",indexChild)
